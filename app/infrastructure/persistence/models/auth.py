...
from app.infrastructure.persistence.models.audit import TransformLog
...
class CasbinRule(SQLModel, table=True, table_description="casbin规则表"):
    """Casbin 规则存储表"""
    __tablename__ = f"{app_settings.APP_NAME}_casbin_rule"  # 应用前缀避免冲突
...
class CasbinSystem(SQLModel, table=True, table_description="casbin 系统定义表"):
    __tablename__ = f"{app_settings.APP_NAME}_casbin_system"
...
class CasbinRole(SQLModel, table=True, table_description="casbin 角色表"):
    __tablename__ = f"{app_settings.APP_NAME}_casbin_role"
...
class SSOSession(SQLModel, table=True, table_description="单点登录"):
    __tablename__ = f"{app_settings.APP_NAME}_sso_session"
    __table_args__ = (
    ...
    user_id: int = Field(..., foreign_key=f"{app_settings.APP_NAME}_user.id", title="关联用户ID")  # 必须绑定用户
    ...
    def generate_device_fingerprint(request: Request):
        """设备指纹生成 — SHA256(ip + UA + Accept-Language + Sec-CH-UA)

        增强指纹（第八阶段）：除 IP、User-Agent 外，
        加入 Accept-Language 与 Sec-CH-UA 提高区分度。
        """
        parts = [
...
class MFASecuity(SQLModel, table=True, table_description="双因素认证表"):
    __tablename__ = f"{app_settings.APP_NAME}_mfa_secuity"

    id: int = Field(..., sa_column=Column(Integer, autoincrement=True, primary_key=True), allow_mutation=False)
    user_id: int = Field(..., foreign_key=f"{app_settings.APP_NAME}_user.id")
    user: "User" = Relationship(back_populates="mfa_secuity")
    method_type: str  # "TOTP", "SMS", "Email"
    secret: str = Field(..., max_length=500, title="加密存储（如TOTP密钥）",
...
class RevokedToken(SQLModel, table=True, table_description="Token失效表"):
    __tablename__ = f"{app_settings.APP_NAME}_revoked_token"
...
class OauthAccount(SQLModel, table=True, table_description="第三方认证账户表"):
    __tablename__ = f"{app_settings.APP_NAME}_oauth_account"
    ...
    verification_token: str = Field(..., max_length=500,
                                    sa_column=Column(StringEncryptedType(type_in=String,
                                                                         key=app_settings.SECRET_KEY,  # 使用配置的密钥
    ...
    user_id: int = Field(..., foreign_key=f"{app_settings.APP_NAME}_user.id", title="UserID")
    user: "User" = Relationship(back_populates="oauth_accounts")
    provider_id: int = Field(..., foreign_key=f"{app_settings.APP_NAME}_oauth_provider.id", title="ProviderID")
...
class OAuthState(SQLModel, table=True, table_description="Oauth State参数"):
    __tablename__ = f"{app_settings.APP_NAME}_oauth_state"
    ...
    async def validate_oauth_state(cls, session: AsyncSession, state: str) -> str:
        ...
        await session.delete(state_recode)
        await session.commit()
...
class OauthProvider(OauthProviderMixin, table=True, table_description="第三方平台认证表"):
    __tablename__ = f"{app_settings.APP_NAME}_oauth_provider"
    ...
    async def transform_fields(self, raw_data: dict, session: AsyncSession) -> dict:
        # 显式查询映射（relationship 懒加载在 async session 中会触发 MissingGreenlet）
        result = await session.exec(
            select(FieldMapping).where(FieldMapping.provider_id == self.id)
        )
        field_mappings = {
        ...
        for key, value in raw_data.items():
            ...
            try:
                ...
                _transform_logger = AsyncLogger.get_logger(**{"name": "transform"})
                _transform_logger.error(f"Transform failed for {key}: {transform_rule} - {str(e)}")
...
class FieldMappingMixin(SQLModel):
    provider_id: int = Field(..., foreign_key=f"{app_settings.APP_NAME}_oauth_provider.id")
    source_field: str
    target_field: str
...
class FieldMapping(FieldMappingMixin, table=True, table_description="第三方认证回传字段映射表"):
    __tablename__ = f"{app_settings.APP_NAME}_field_mapping"
